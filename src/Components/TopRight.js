...
const TopRight = ({close, viewers, kActived, setClose, setMic, mic, setCam, cam, setFacingMode, initViewers, setViewers}) => {
  ...
  useEffect(() => {
  const refreshTime = Math.floor(Math.random() * 5000) + 4000;
  const randomNum = Math.floor(Math.random() * (initViewers / 2)) / 10;
  
  const updateViewers = setInterval(() => {
    const plusOrLess = Math.floor(Math.random() * 2);

    if (viewers <= initViewers * 0.75 && plusOrLess === 0) {
      setViewers(viewers + randomNum);
    } else if (viewers >= initViewers * 0.75 && plusOrLess === 1) {
      setViewers(viewers - randomNum);
    }
  }, refreshTime);
        ...
        <div id='top-right' className={close === true ? 'hidden' : ''}>
          <div>
            ...
            <small id='viewers'>
              ...
              {viewers && parseFloat(viewers.toFixed(1))}{kActived && 'k'}
            </small>
